...
Usage:
  ...
  python run_all_experiments.py --example 2  # run one example (1-6)
...
def main():
    ...
    parser.add_argument("--example", type=int, default=0,
                        help="Run only example N (1-6). 0 = run all.")
    parser.add_argument("--skip-images", action="store_true",
                        help="Skip image deblurring examples (4.3, 4.4, 4.5)")
    args = parser.parse_args()

    if args.example:
        if args.example not in DISPATCH:
            print(f"Unknown example {args.example}. Choose 1-6.")
            sys.exit(1)
        DISPATCH[args.example]()
    else:
        for num, fn in DISPATCH.items():
            if args.skip_images and num in (3, 4, 5):
