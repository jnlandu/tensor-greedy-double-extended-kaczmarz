...
\input{TGDBEK.sty}
\date{AIMS RIC, Doctoral Training School - 2026}
...
\begin{frame}{Why Tensors?}
  \begin{columns}[T]
    ...
    \begin{column}{0.4\textwidth}
      \centering
      \includegraphics[width=\linewidth]{image2tensor.png}
      \\[4pt]
      {\small Image as a 3D tensor (H x W x C)}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{figure}
  ...
  \begin{subfigure}[b]{0.48\linewidth}
    \centering
    % \includegraphics[width=\linewidth]{3d-tensor.png}
      \includegraphics[width=\linewidth]{tensors-v.jpg}
    % \caption{Caption}
    % \label{fig:3d-tensor-1}
  \end{subfigure}\hfill
  \begin{subfigure}[b]{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{3d-tensor-1.png}
    % \caption{Caption}
...
%-------------------------
\section{The t-Product}
\begin{frame}{The t-Product: Why and How}

  \textbf{Definition} (Kilmer \& Martin, 2011): Given $\mcalA \in \mbbr^{m \times n \times p}$
  and $\mcalX \in \mbbr^{n \times k \times p}$, the t-product $\mcalA * \mcalX$ is:
  \[
    \mcalA * \mcalX
    \;=\;
    \fold\!\Big(\bcirc(\mcalA) \cdot \unfold(\mcalX)\Big)
    \;\in\; \mbbr^{m \times k \times p}
  \]
  where $\bcirc(\mcalA) \in \mbbr^{mp \times np}$ is the block-circulant matrix of $\mcalA$, and $\unfold(\mcalX) \in \mbbr^{nk \times p}$ is the mode-3 unfolding of $\mcalX$, defined as:
...
\begin{frame}
    \textbf{Why the t-product?}
  \begin{itemize}
    % \item Vectoriziation don't capture the multi-dimensional  structure of tensors and  leads to loss of information and interpretability.
    \item Classical matrix algebra cannot directly capture the \textbf{multi-channel structure}
          of tensors (e.g.\ RGB channels, video frames).
    \item The t-product defines a \textbf{matrix-like algebra for third-order tensors},
          preserving notions of transpose, inverse, SVD, and rank.
  \end{itemize}
\end{frame}
\begin{frame}
   \medskip
  ...
  \begin{itemize}
    \item Efficient computation via \textbf{FFT along the tube dimension}
    \item Supports transpose ($\mcalA^\top$), pseudoinverse ($\mcalA^\dagger$), and t-SVD
    \item Enables a full \textbf{linear algebra framework} for 3D data
  \end{itemize}
\end{frame}
...
\begin{frame}{Cyclic and Randomized Kaczmarz}
  Consider a linear system $Ax=b$ with rows $a_i^\top$ of $A$.

  \medskip
  \textbf{Kaczmarz update (row projection):}
  \[
    x^{k+1} = x^k + \frac{b_{i_k} - a_{i_k}^\top x^k}{\|a_i\|_2^2} a_{i_k}
  \]
  \begin{itemize}
    \item At each step, project $x^k$ onto the hyperplane
          $\{x: a_i^\top x = b_i\}$.
    \item Cyclic version scans rows in order $i_k = k(\mod i) + 1 $ % 1,2,\dots,m$.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Pick row $i_k$ randomly, with
          $\Pr(i_k=i)=\|a_i\|_2^2/\|A\|_F^2$.
    \item Same projection formula with $i=i_k$.
    \item For consistent and overdetermined  systems, RK converges \emph{linearly in expectation} and satisfies:
          \[
            \mathbb{E}[\|x^k - x^\star\|_2^2] \leq \left(1 - \frac{\sigma_{\min}^2(A)}{\|A\|_F^2}\right)^k \|x^0 - x^\star\|_2^2
          \]
    \item For inconsistent systems $A  x = b + \epsilon$ , the \textbf{randomized extended Kaczmarz (REK)}  variant  applies doubly the RK method to the systems 
    $$ A x = b-z  \quad A^\top z = 0 $$.

    
    \item converges linearly in expectation to the minimum-norm  \emph{least-squares solution}.
  \end{itemize}
\end{frame}
...
\begin{frame}
  \begin{center}
    \begin{figure}
      \includegraphics[width=0.8\textwidth]{kaczmarz_projection.png}
    \caption{Illustration of Kaczmarz iterations as projections onto hyperplanes defined by the rows of $A$.}
    \end{figure}
  \end{center}
\end{frame}
...
\begin{frame}{Problem Setup}
  ...
  \textbf{Setting:}
  \begin{itemize}
    \item System is \textbf{inconsistent} ($\mcalB \notin \ran(\mcalA)$)
...
\begin{frame}{Existing Methods \& Limitations}
  ...
  \begin{itemize}
    \item \textbf{TREK} -- Tensor Randomized Extended Kaczmarz \cite{huang2024tensor}
    \item \textbf{TREBK} -- Tensor Randomized Extended Block Kaczmarz \cite{huang2024tensor}
    \item \textbf{TREGBK} -- Tensor Randomized Extended Greedy Block Kaczmarz \cite{huang2024tensor}
    \item \textbf{TREABK} -- Tensor Randomized Extended Averaged Block Kaczmarz \cite{an2025randomized,bao2022randomized}
  \end{itemize}
\end{frame}
...
\begin{frame}{Convergence Guarantee}
  \begin{theorem} 
    Let $\mcalA \in \mbbr^{m \times n \times p}$ and $\mcalB \in \mbbr^{m \times k \times p}$. let $\fm B^\perp := \fm B - \fm A *\fm A^\dagger *\fm B$. The iterate $\{\fm X^{(k)}\}$ generated  by TGDBEK algorithm  converges
    \text{linearly} to the minimum-norm least-square solution $\fm X_\ast = \fm A^\dagger * \fm B$. Moreover, for $k\geq 1$, the following holds
\begin{align*}
...
\begin{frame}
  % \textbf{Key points:}
  \begin{itemize}
    \item  {\small \textit{Full proof available in the technical report.}}
  \end{itemize}
\end{frame}
...
\begin{frame}[shrink=10]{Results on Sparse Matrices (SuiteSparse)}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      \centering
      \includegraphics[width=.9\linewidth]{ash85_tdgbek.png}
      \\[1pt]
      {\scriptsize RSE vs.\ Iterations (ash85)}

      \medskip
      \includegraphics[width=.9\linewidth]{nos5_tdgbek.png}
      \\[1pt]
...
\begin{frame}{}
  \begin{columns}[c]
    \begin{column}{0.5\textwidth}
       \centering
      {\small \textbf{Cities Matrix} ($128 \times 128$), density $= 0.072$)}
      \begin{tabular}{@{}lrrr@{}}
        \toprule
        \textbf{Method} & \textbf{Time} & \textbf{RSE} & \textbf{IT} \\
        \midrule
...
\begin{frame}[shrink=5]{Results on Color Image Deblurring \& Reconstruction}
  {\small Color image deblurring: TGDBEK achieves higher PSNR and faster convergence
  than TREK/TREBK/TREGBK on standard test images.}

  \smallskip
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[width=0.75\linewidth]{flower.jpg}
      \\[3pt]
      {\small Original image}
    \end{column}
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[width=0.75\linewidth]{ex3-blur.png}
      \\[3pt]
      {\small Blurred image}
    \end{column}
  \end{columns}
...
\section{Experiments on Image reconstruction}
\begin{frame}{Experiments on Image reconstruction \& Color image deblurring}
